        ...
        if($bookInfo): // 유효성 확인
            ...
            <div id="main-center-center">
                <div id="bookDetail">
                    ...
                    <?php if ($bookInfo['availability']): ?>
                        <?php if ($loggedIn): ?>
                            <button onclick="loanBook(<?= $bookInfo['isbn'] ?>)">대출하기</button>
                        <?php else: ?>
                            <button onclick="alert('로그인 이후 대출가능합니다')">대출하기</button>
                        <?php endif; ?>
                    <?php else:
                    // 본인이 대출한 책인지 확인
                    if($loggedIn):
                        $stmt = $pdo->prepare("SELECT * FROM loan_records WHERE isbn = ? AND user_id = ? AND returned = 0");
                        $stmt->execute([$bookInfo['isbn'], $_SESSION['id_num']]);
                        $loanRecord = $stmt->fetch();
                            if ($loanRecord): ?>
                                <button onclick="returnBook(<?= $bookInfo['isbn'] ?>)">반납하기</button>
                            <?php else: ?>
